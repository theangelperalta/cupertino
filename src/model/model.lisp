...
(defclass cupertino-model (model-interface)
  ((project-type :initarg :project-type :accessor model-project-type)
   ...
   (schemes :initarg :schemes :accessor model-schemes)
   (test-schemes :initarg :test-schemes :accessor model-test-schemes)
   (cells :initarg :cells :accessor model-cells)
   (test-cells :initarg :test-cells :accessor model-test-cells)
...
(defun make-cupertino-model (&optional path)
  ...
  (let* ((plist (load-model path))
    ...
    (make-instance 'cupertino-model
                   ...
                   :schemes (getf plist :schemes)
                   :test-schemes (getf plist :test-schemes)
                   :cells (getf plist :cells)
                   :test-cells (getf plist :test-cells)
